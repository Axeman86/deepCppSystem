...
public:
    ...
    Char * expandNoinit(const size_t delta, bool expGrowth = false, bool disableSSO = FBSTRING_DISABLE_SSO);
    ...
    //是否copy-on-write 共享
    bool isShared() const { return category() == Category::isLarge && RefCounted::refs(ml_.data_) > 1; }
...
private:
    ...
    struct RefCounted
    {
        ...
        constexpr static size_t getDataOffset() { return offsetof(RefCounted, data_); }
        ...
        static RefCounted * reallocate(Char * const data, const size_t currentSize, const size_t currentCapacity,
                                       size_t * newCapacity)
        {
            ...
            auto result =
                static_cast<RefCounted *>(smartRealloc(dis, getDataOffset() + (currentSize + 1) * sizeof(Char),
                                                       getDataOffset() + (currentCapacity + 1) * sizeof(Char), allocNewCapacity));
    ...
    struct MediumLarge
    {
        ...
        size_t capacity() const { return kIsLittleEndian ? capacity_ & capacityExtractMask : capacity_ >> 2; }

        void setCapacity(size_t cap, Category cat)
        {
            capacity_ =
                kIsLittleEndian ? cap | (static_cast<size_t>(cat) << kCategoryShift) : (cap << 2) | static_cast<size_t>(cat);
        }
    };
    ...
    constexpr static size_t capacityExtractMask =
        kIsLittleEndian ? ~(size_t(categoryExtractMask) << kCategoryShift) : 0x0 /* unused */;

    static_assert(!(sizeof(MediumLarge) % sizeof(Char)), "Corrupt memory layout for fbstring.");
...
//初始化medium字符串
template <class Char>
FOLLY_NOINLINE void fbstring_core<Char>::initMedium(const Char * const data, const size_t size)
{
...
//初始化large字符串
template <class Char>
FOLLY_NOINLINE void fbstring_core<Char>::initLarge(const Char * const data, const size_t size)
{
...
template <class Char>
FOLLY_NOINLINE void fbstring_core<Char>::reserveMedium(const size_t minCapacity)
{
    assert(category() == Category::isMedium);
    ...
    else
    {
        ...
        // Also copies terminator.
        fbstring_detail::podCopy(ml_.data_, ml_.data_ + ml_.size_ + 1, nascent.ml_.data_);
        nascent.swap(*this);
        assert(capacity() >= minCapacity);
    }
}
...
template <class Char>
FOLLY_NOINLINE void fbstring_core<Char>::reserveSmall(size_t minCapacity, const bool disableSSO)
{
    assert(category() == Category::isSmall);
...
template <class Char>
inline Char * fbstring_core<Char>::expandNoinit(const size_t delta, bool expGrowth, /* = false */
